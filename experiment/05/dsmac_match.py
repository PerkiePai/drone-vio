#!/usr/bin/env python3
"""DSMAC-style vision-only geo-localization — extractor comparison (Exp05).

Absolute (drift-free) position by matching each nadir frame against a pre-stored
satellite ortho. Per frame: de-rotate by -yaw, scale by AGL/fx/GSD to the ortho's
m/px, match with <extractor>+LightGlue, RANSAC-homography the frame centre onto the
ortho, convert the hit pixel back to ENU and score against geo.csv.

Exp05 additions vs Exp03:
  --extractor {aliked,superpoint,disk,sift}  swap the LightGlue front-end
  --n_sample N   sample N frames evenly across the full flight (default 200)
  --benchmark    profile per-fix sub-step latency (unchanged from Exp03)
...
ROOT = os.path.dirname(os.path.dirname(HERE))  # experiment/05 -> repo root
sys.path.insert(0, HERE)  # flow_odometry.py is co-located
import flow_odometry as fo

TILE_URL = ("https://services.arcgisonline.com/ArcGIS/rest/services/"
            "World_Imagery/MapServer/tile/{z}/{y}/{x}")

FIX_BUDGET_MS = 2400.0  # 30 frames × 80 ms
DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
MAX_KPTS = 2048

EXTRACTORS = ["aliked", "superpoint", "disk", "sift"]


def make_lg(extractor_name="aliked"):
    """Return a matcher object with .extract(gray) and .match(f0, f1) for the given extractor."""
    from lightglue import LightGlue
    from lightglue.utils import rbd

    if extractor_name == "aliked":
        from lightglue import ALIKED
        ext = ALIKED(max_num_keypoints=MAX_KPTS).eval().to(DEVICE)
    elif extractor_name == "superpoint":
        from lightglue import SuperPoint
        ext = SuperPoint(max_num_keypoints=MAX_KPTS).eval().to(DEVICE)
    elif extractor_name == "disk":
        from lightglue import DISK
        ext = DISK(max_num_keypoints=MAX_KPTS).eval().to(DEVICE)
    ...
    mat = LightGlue(features=extractor_name).eval().to(DEVICE)
    print(f"  loaded {extractor_name.upper()}+LightGlue on {DEVICE}")
...
def _print_benchmark(bm_tile, bm_detect, bm_match, bm_ransac, bm_total, extractor):
    steps = [
        ("tile/cache",   bm_tile),
        (f"{extractor} detect", bm_detect),
        ("LightGlue",    bm_match),
        ("RANSAC",       bm_ransac),
        ("TOTAL",        bm_total),
    ]
    print(f"\n{'─'*66}")
    print(f"  BENCHMARK [{extractor}+LG] — per-fix latency (ms)  "
          f"[budget {FIX_BUDGET_MS:.0f} ms / 30 frames]")
    print(f"{'─'*66}")
    print(f"  {'Sub-step':<20} {'N':>5} {'Mean':>8} {'P95':>8} {'Max':>8}")
    print(f"{'─'*66}")
    for name, vals in steps:
        if not vals:
            continue
        a = np.array(vals)
        print(f"  {name:<20} {len(a):>5} {a.mean():>8.1f} {np.percentile(a, 95):>8.1f} {a.max():>8.1f}")
    print(f"{'─'*66}")
    if bm_total:
        tot = np.array(bm_total)
        headroom = FIX_BUDGET_MS - tot.mean()
        over = int((tot > FIX_BUDGET_MS).sum())
        print(f"  Fix budget headroom : {headroom:.0f} ms  ({'OK' if headroom > 0 else 'FAIL'})")
        print(f"  Over-budget fixes   : {over}/{len(tot)}  ({100 * over / max(len(tot), 1):.1f}%)")
        print(f"  Must background?    : {'YES — fix latency > 80 ms' if tot.mean() > 80 else 'NO (fits in 1 frame)'}")
    print(f"{'─'*66}\n")
...
def main():
    ...
    ap.add_argument("--n_sample", type=int, default=200,
                    help="frames sampled evenly across the flight")
    ...
    ap.add_argument("--benchmark", action="store_true",
                    help="profile per-fix sub-step latency (tile fetch, extract, LightGlue, RANSAC)")
    args = ap.parse_args()
    D = args.dir
    ds_name = os.path.basename(D)
    out_dir = os.path.join(HERE, ds_name)
    os.makedirs(out_dir, exist_ok=True)
    out = args.out or os.path.join(out_dir, f"dsmac_geoloc_{args.extractor}.png")
    ...
    idxs = np.linspace(50, len(recs) - 50, args.n_sample).astype(int)
    res = []  # i, gE, gN, eE, eN, inliers, err_m

    if args.benchmark:
        bm_tile, bm_detect, bm_match, bm_ransac, bm_total = [], [], [], [], []

    print(f"\nrunning {args.n_sample} frames with {args.extractor.upper()}+LightGlue ...")
    for fi in idxs:
        r = recs[fi]
        ...
        try:
            ...
            if args.benchmark:
                ...
                _t0 = time.perf_counter()
            k0, k1, _ = lg.match(kp_q, kp_w)
            if args.benchmark:
                bm_match.append((time.perf_counter() - _t0) * 1000)
            ...
            if len(k0) >= 8:
                ...
                if args.benchmark:
                    bm_ransac.append((time.perf_counter() - _t0) * 1000)
                ...
                bm_ransac.append(0.0)
        except Exception:
            if args.benchmark:
                bm_detect.append(0.0); bm_match.append(0.0); bm_ransac.append(0.0)
    ...
    print(f"\nextractor       : {args.extractor.upper()}+LightGlue")
    print(f"frames tried    : {len(res)}  matched {len(acc)} = {100*len(acc)/len(res):.0f}%")
    if len(acc):
        e = acc[:, 6]
        print(f"abs-pos error   : median {np.median(e):.1f} m  mean {e.mean():.1f} m  "
              f"p90 {np.percentile(e,90):.1f} m  max {e.max():.1f} m")
        print(f"success         : <30m {100*np.mean(e<30):.0f}%  "
              f"<50m {100*np.mean(e<50):.0f}%  <100m {100*np.mean(e<100):.0f}%")

    if args.benchmark:
        _print_benchmark(bm_tile, bm_detect, bm_match, bm_ransac, bm_total, args.extractor)
    ...
    a.set_title(f"DSMAC fixes vs GT — {args.extractor.upper()}+LightGlue")
    ...
    fig.suptitle(f"DSMAC geo-localization — {args.extractor.upper()}+LightGlue — {ds_name}",
                 fontsize=12)
    fig.tight_layout()
