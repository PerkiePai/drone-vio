...
def make_lg(extractor_name="sift"):
    """Return a <extractor>+LightGlue matcher with .extract(gray) and .match(f0, f1)."""
    ...
    print(f"  {extractor_name.upper()}+LightGlue loaded on {DEVICE}")
...
def report_and_plot(fused, GT, tvec, fixes, n_used, args):
    ...
    stats = (
        ...
        f"{'DSMAC extractor':<22} {args.extractor.upper()}+LightGlue\n"
...
def main():
    ...
    ap.add_argument("--extractor",   choices=["sift", "aliked"], default="sift",
                    help="DSMAC LightGlue front-end extractor (default sift)")
    ...
    ap.add_argument("--out",         default=None,
                    help="output plot path (default: <HERE>/<dataset>/fused_ahrs_compass_agl_rej<R>_inl<N>_<extractor>.png)")
    args = ap.parse_args()
    ...
    if args.out is None:
        ...
        rej_tag = "auto" if args.autotune else f"{args.reject:.0f}"
        args.out = os.path.join(
            out_dir,
            f"fused_ahrs_compass_agl_rej{rej_tag}_inl{args.min_inliers}_{args.extractor}.png")
