...
Exp03 addition: --benchmark flag profiles per-frame sub-step latency.
"""
import argparse
import csv
import json
...
FRAME_BUDGET_MS = 80.0  # 12.5 Hz = 80 ms per frame
...
def _print_benchmark(bm_load, bm_feat, bm_flow, bm_fb, bm_solve, bm_total):
    steps = [
        ("load",      bm_load),
        ("feat",      bm_feat),
        ("flow",      bm_flow),
    ...
    print(f"\n{'─'*62}")
    print(f"  BENCHMARK — per-frame latency (ms)   [budget {FRAME_BUDGET_MS:.0f} ms @ 12.5 Hz]")
    print(f"{'─'*62}")
    print(f"  {'Sub-step':<14} {'Mean':>8} {'P95':>8} {'Max':>8}  {'Target'}")
    targets = {"load": "<5", "feat": "<5", "flow": "<10", "fb-check": "<3", "solve": "<2", "TOTAL": "<20"}
    print(f"{'─'*62}")
    for name, vals in steps:
        ...
        print(f"  {name:<14} {a.mean():>8.1f} {np.percentile(a, 95):>8.1f} {a.max():>8.1f}  {tgt}")
    print(f"{'─'*62}")
...
def run(d, scale, max_frames, min_track, depth_source="baro", stride=1, fb_check=True,
        agl_arr=None, attitude_R=None, skip_frames=0, benchmark=False):
    ...
    if benchmark:
        bm_load, bm_feat, bm_flow, bm_fb, bm_solve, bm_total = [], [], [], [], [], []
    ...
    for i in range(stride, len(recs), stride):
        ...
        if benchmark:
            bm_feat.append((time.perf_counter() - _t0) * 1000)
        ...
        if p0 is not None and len(p0) >= min_track:
            ...
            if benchmark:
                bm_flow.append((time.perf_counter() - _t0) * 1000)
            ...
            if fb_check:                     # forward-backward consistency: re-track
                ...
                if benchmark:
                    bm_fb.append((time.perf_counter() - _t0) * 1000)
    ...
    if benchmark:
        _print_benchmark(bm_load, bm_feat, bm_flow, bm_fb, bm_solve, bm_total)
...
if __name__ == "__main__":
    ...
    ap.add_argument("--benchmark", action="store_true",
                    help="profile per-frame sub-step latency and print summary table")
    args = ap.parse_args()
    if args.out:
        out = args.out
    else:
        EXP_DIR = os.path.dirname(os.path.abspath(__file__))  # experiment/03
    ...
    est, gt, n_used, impl_depth, recs = run(args.dir, args.scale, args.max_frames,
                                            ...
                                            attitude_R=att_R, benchmark=args.benchmark)
