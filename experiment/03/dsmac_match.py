...
from geo.csv; cached under <out>/ortho_tiles. Run in the `drone` conda env (torch +
...
Exp03 addition: --benchmark flag profiles per-fix sub-step latency and prints a
summary table (tile-fetch/cache, ALIKED detect, LightGlue match, RANSAC homography).
"""
...
ROOT = os.path.dirname(os.path.dirname(HERE))  # experiment/03 -> repo root
...
def _print_benchmark(bm_tile, bm_detect, bm_match, bm_ransac, bm_total):
    steps = [
        ("tile/cache",   bm_tile),
        ("ALIKED detect", bm_detect),
        ("LightGlue",    bm_match),
        ("RANSAC",       bm_ransac),
        ("TOTAL",        bm_total),
    ]
    targets = {
        "tile/cache":    "<10 (cached)",
        "ALIKED detect": "100–300",
        "LightGlue":     "100–400",
        "RANSAC":        "<20",
        "TOTAL":         "<800",
    }
    print(f"\n{'─'*66}")
    print(f"  BENCHMARK — per-fix latency (ms)   [budget {FIX_BUDGET_MS:.0f} ms / 30 frames]")
    print(f"{'─'*66}")
    print(f"  {'Sub-step':<18} {'N':>5} {'Mean':>8} {'P95':>8} {'Max':>8}  {'Target'}")
    print(f"{'─'*66}")
    for name, vals in steps:
        ...
        print(f"  {name:<18} {len(a):>5} {a.mean():>8.1f} {np.percentile(a, 95):>8.1f} {a.max():>8.1f}  {tgt}")
    print(f"{'─'*66}")
    if bm_total:
...
def main():
    ...
    ap.add_argument("--benchmark", action="store_true",
                    help="profile per-fix sub-step latency (tile fetch, ALIKED, LightGlue, RANSAC)")
    args = ap.parse_args()
    D = args.dir
    ...
    if args.benchmark:
        _print_benchmark(bm_tile, bm_detect, bm_match, bm_ransac, bm_total)
