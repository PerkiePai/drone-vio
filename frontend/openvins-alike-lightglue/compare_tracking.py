...
Compares the OpenVINS DEFAULT front-end against a learned one on the SAME frames:

  - KLT     : FAST corners + pyramidal Lucas-Kanade optical flow  (what OpenVINS uses)
  - ALIKED+LightGlue : learned detector + learned matcher

Two views, both VIO-relevant:
  1. Pairwise vs frame-gap g in {1,2,3,5,10}  -> robustness to baseline / fast motion.
        matches, RANSAC-fundamental inliers, inlier ratio, latency.
  2. Track survival over consecutive frames    -> track length (MSCKF wants long tracks).
        features seeded on frame 0, how many survive to frame k.
...
# ---------------- experiments ----------------
def pairwise(frames, scale, gaps, lg, n_pairs):
    grays = {}
    def G(i):
...
def survival(frames, scale, lg, T):
    """Seed on frame 0, count survivors to frame k for both methods."""
    ...
    track = {j: j for j in range(rbd(feats[0])["keypoints"].shape[0])}  # cur_kpt_idx -> origin
    lg_surv = [len(track)]
    for t in range(1, T + 1):
...
def make_viz(frames, scale, lg, rows, klt, lgs):
    import matplotlib
    matplotlib.use("Agg")
    ...
    for gap in (1, 10):
        ...
        cv2.imwrite(os.path.join(OUT, f"matches_gap{gap}.png"), np.vstack([a, b]))
    # --- survival curve ---
    ...
    for name, mk in (("KLT", "o"), ("ALIKED+LightGlue", "s")):
...
def main():
    ...
    lg = LG()
    # warmup
    _ = lg.match(lg.extract(load(frames[0], args.scale)[1]), lg.extract(load(frames[1], args.scale)[1]))

    print("=== pairwise vs frame-gap (mean over pairs) ===")
    print(f"{'method':>18} {'gap':>4} {'matches':>8} {'min':>5} {'inliers':>8} {'ratio':>6} {'ms':>7}")
    rows = pairwise(frames, args.scale, args.gaps, lg, args.pairs)
    for r in rows:
    ...
    n0, klt, lgs = survival(frames, args.scale, lg, args.surv_T)
    print(f"KLT seeds={klt[0]}   ALIKED+LG seeds={lgs[0]}")
    print(f"{'frame':>6} {'KLT':>8} {'KLT%':>6} {'ALIKED+LG':>10} {'ALG%':>6}")
    ...
    with open(os.path.join(OUT, "survival.csv"), "w", newline="") as f:
        cw = csv.writer(f); cw.writerow(["frame", "KLT", "ALIKED_LightGlue"])
        for k in range(len(klt)): cw.writerow([k, klt[k], lgs[k]])
    print(f"\nCSVs -> {OUT}/pairwise.csv, survival.csv")

    if args.viz:
        make_viz(frames, args.scale, lg, rows, klt, lgs)
